                        ...
                        <table id="adminsTable" class="table table-hover align-middle mb-0 w-100">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            ...
                                            <img src="{{ asset('storage/' . $admin->profile_pic) }}"
                                                ...
                                                onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name={{ urlencode($admin->nama) }}&color=7F9CF5&background=EBF4FF';">
                                            @else
                                            <div class="avatar-sm me-3 bg-primary-subtle rounded-circle d-flex align-items-center justify-content-center" style="width: 35px; height: 35px; flex-shrink: 0;">
                                                <span class="text-primary fw-bold small">{{ substr($admin->nama, 0, 1) }}</span>
                                            </div>
                                            @endif
                                            <div>
                                    ...
                                    <td>
                                        <div class="small fw-medium">{{ $admin->created_at ? $admin->created_at->format('d M Y') : '-' }}</div>
                                    </td>
                                    <td class="text-end pe-3">
                                        <div class="d-flex gap-1 justify-content-end">
                                            <button class="btn btn-sm btn-white border shadow-sm px-2" title="Edit" onclick="openEditModal('{{ $admin->id }}')">
                                                <i class="ti ti-edit text-primary"></i>
                                            </button>
                                            <form method="POST" action="{{ route('admin.admins.destroy', $admin->id) }}" class="d-inline" onsubmit="return confirm('Hapus admin ini?');">
                                                @csrf @method('DELETE')
