    ...
    <main id="main-content">
        <div class="card welcome-card mb-4 border-0 shadow-lg position-relative overflow-hidden rounded-4">
            <div class="card-body p-4 p-md-5">
                ...
                <img src="{{ asset('assets/img/aspira4.png') }}" class="welcome-img d-none d-md-block" alt="Illustration">
            </div>
        </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 h-100 card-hover">
                        <div class="card-body p-4">
                            <div class="d-flex justify-content-between align-items-start mb-3">
                                <span class="badge {{ $item->status == 'Selesai' ? 'bg-success-subtle text-success' : ($item->status == 'Proses' ? 'bg-primary-subtle text-primary' : 'bg-warning-subtle text-warning') }} border-0 px-3 py-2 rounded-pill small fw-bold">
                                    {{ strtoupper($item->status ?? 'MENUNGGU') }}
                                </span>
                            ...
                            <div class="mb-4">
                                ...
                                <p class="mb-0 text-muted x-small">
                                    <i class="ti ti-clock me-1"></i> {{ $item->created_at->format('d M Y, H:i') }}
                                </p>
                            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        new Chart(ctx, {
            type: 'line',
            data: {
                labels: JSON.parse('{!! json_encode($chart_labels) !!}'),
                datasets: [{
                    label: 'Laporan',
                    data: JSON.parse('{!! json_encode($chart_data) !!}'),
                    fill: true,
